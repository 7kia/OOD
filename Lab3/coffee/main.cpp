...
int main()
{
				...
				make_unique<CIceCubes>(				// | под нею - кубики льда
					make_unique<CLemon>(			// | | еще ниже лимон
						make_unique<CCinnamon>(		// | | | слоем ниже - корица
							make_unique<CLatte>(CLatte::Type::OnePortion)),	// | | |   в самом сердце - Латте
						2),							// | | 2 дольки лимона
	...
	// Подробнее рассмотрим работу MakeCondiment и оператора <<
	{
		...
		*/
		...
		auto oneMoreLemonIceTea =
			make_unique<CTea>(CTea::SortType::Black)	// Берем чай
	...
	{
		auto beverage = 
			make_unique<CLatte>(CLatte::Type::OnePortion)							// Наливаем чашечку латте,
	...
	{
		auto beverage = 
			make_unique<CMilkshake>(CMilkshake::PortionType::Small)					// Наливаем молочный коктейль
